...
export function Test() {
  ...
  return (
    <div className='bg-slate-50 w-full relative h-[200px] gap-4 items-center flex-col justify-center flex rounded-md shadow border'>
      <div className='flex items-center justify-center gap-4'>
        {radios.map((radio) => (
          <div key={radio.id} className='flex items-center gap-2 relative'>
            <input type="radio" name="radio" id={radio.value} value={radio.value} className='hidden' />
            <motion.div
              onClick={() => setSelected(radio.value)}
              className='w-4 h-4 bg-white border border-black rounded-full relative cursor-pointer'>
              {selected === radio.value &&
                <motion.div layoutId="radio-circle"
                  initial={false}
                  animate={{
                    scale: [1, 2, 2, 1],
            ...
            <label
              onClick={() => setSelected(radio.value)}
              htmlFor={radio.value}>{radio.name}</label>
          </div>
        ))}
      </div>
      <div className='flex items-center flex-col justify-center gap-1'>
        <p className='text-sm'>{selected}</p>
        <p>A simple radio button made with simple animations.</p>
      </div>
    </div>
  )
}
